#ifndef iAItkBinaryDilateImageFilter_hxx
#define iAItkBinaryDilateImageFilter_hxx
...
#include "iABinaryDilateImageFilter.h"

namespace itk
{
	template< class TInputImage, class TInputImageType2, class TOutputImage, class TKernel >
	iABinaryDilateImageFilter< TInputImage, TInputImageType2, TOutputImage, TKernel >
		...
		::GenerateData()
	{
		...
		//New code for the adaptation. Beginning //
		typename TInputImageType2::Pointer labelImage;
		labelImage = dynamic_cast<TInputImageType2*>(ProcessObject::GetInput(1));

		typename TInputImageType2::SizeType radiusFCPI;
		radiusFCPI[0] = 1;
		radiusFCPI[1] = 1;
		radiusFCPI[2] = 1;

		itk::NeighborhoodIterator<TInputImageType2> iteratorFCPI(radius, labelImage, labelImage->GetRequestedRegion()); //Iterator for the label image. This is to prevent 
		//Adaptation. End //
			...
			if ( tmpRegIndexIt.Get() == onTag )
			{
				...
				if ( bIsOnContour )
				{
						...
						++itIdx )
					{
						...
						{
							//Find out if idx can be set. This is the case if there are no two voxels with different
							//values in our one radius neighbourhood. (26 voxels)

							iteratorFCPI.SetLocation(idx);	//Center of the neighbourhood
							int connectedComponent1 = 0;		//Variable for the first voxel not beeing zero.
							int connectedComponent2 = 0;		//Variable for the second voxel not beeing zero. 
							//If these two exist and are different then the voxel in the center cannot be set.
							bool canBeSet = true;				//Default value is true.

							for(unsigned int i=0; i<iteratorFCPI.Size(); i++)
							{
								if(iteratorFCPI.GetPixel(i)>0)
								{
									if(connectedComponent1 == 0)
										connectedComponent1 = iteratorFCPI.GetPixel(i);
									else
										connectedComponent2 = iteratorFCPI.GetPixel(i);

									if(connectedComponent2>0 && connectedComponent1>0 && connectedComponent2 != connectedComponent1) //If different then the loop will stop.
									{
										canBeSet = false;
										connectedComponent2 = 0;
										connectedComponent1 = 0;
										break;
									}
								}
							}
							...
							if(canBeSet)
							{
								output->SetPixel( idx, static_cast< OutputPixelType >( connectedComponent1 ) ); //Setting value to the output image
								iteratorFCPI.SetCenterPixel( static_cast< OutputPixelType >( connectedComponent1 ) ); //Setting value to the label image
							}
						}
					}
			...
			while ( !ouRegIndexIt.IsAtEnd() )
			{
				...
				for ( vecIt = vecBeginIt; vecIt != vecEndIt; ++vecIt )
				{
						...
						|| input->GetPixel(translatedIndex) == foregroundValue )
					{
						...
						iteratorFCPI.SetLocation(ouRegIndexIt.GetIndex());	//Center of the neighbourhood
						int connectedComponent1 = 0;		//Variable for the first voxel not beeing zero.
						int connectedComponent2 = 0;		//Variable for the second voxel not beeing zero. 
						//If these two exist and are different then the voxel in the center cannot be set.
						bool canBeSet = true;				//Default value is true.
						...
						if(canBeSet)
						{
							ouRegIndexIt.Set( static_cast< OutputPixelType >( connectedComponent1 ) ); //Setting value to the output image
							iteratorFCPI.SetCenterPixel( static_cast< OutputPixelType >( connectedComponent1 ) ); //Setting value to the label image
						}
	...
	*/
	template< class TInputImage, class TInputImageType2, class TOutput, class TKernel >
	void
		iABinaryDilateImageFilter< TInputImage, TInputImageType2, TOutput, TKernel >
