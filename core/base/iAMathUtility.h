...
base_API double gaussian(double x, double sigma);

//! Compute a gaussian kernel with the given sigma (mean = 0).
base_API FuncType gaussianKernel(double kernelSigma, size_t kernelSteps);
...
base_API FuncType gaussianSmoothing(FuncType const& data, double kernelSigma, int kernelSteps);

//! Calculate first derivative of a given function.
base_API FuncType derivative(FuncType const& func);

//! Compute the mean of a function.
base_API double mean(FuncType const& func);

//! Compute the variation of a function. If known, mean can be given (to improve speed).
base_API double variance(FuncType const & func, double meanVal = std::numeric_limits<double>::infinity(), bool correctDF = true);

//! Compute the standard deviation of a function. mean can be given (to improve speed).
base_API double standardDeviation(FuncType const & func, double meanVal = std::numeric_limits<double>::infinity(), bool correctDF = true);

//! Compute covariance between two functions.
base_API double covariance(FuncType const & func1, FuncType const & func2,
...
base_API FuncType getNormedRanks(FuncType const& func);

//! Calculate the Pearson's correlation coefficient between two functions.
base_API double pearsonsCorrelationCoefficient(FuncType const & func1, FuncType const & func2);

//! Calculate the Spearman's correlation coefficient between two functions.
base_API double spearmansCorrelationCoefficient(FuncType const& func1, FuncType const& func2);
