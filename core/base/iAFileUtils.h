...
base_API QString MakeAbsolute(QString const & baseDir, QString const & fileName);
base_API QString MakeRelative(QString const & baseDir, QString const & fileName);

base_API void FindFiles(QString const& directory, QStringList const& filters, bool recurse,
	QStringList & filesOut, QFlags<FilesFolders> filesFolders);

base_API std::string getLocalEncodingFileName(QString const& fileName);

base_API QString fileNameOnly(QString const& fileName);
...
base_API QString pathFileBaseName(QFileInfo const& fi);
...
base_API void determineStackParameters(QString const & fullFileName,
