...
base_API itk::ImageIOBase::IOComponentType itkScalarPixelType(iAITKIO::ImagePointer image);
base_API itk::ImageIOBase::IOPixelType itkPixelType( iAITKIO::ImagePointer image );
base_API iAITKIO::ImagePointer allocateImage(iAITKIO::ImagePointer img);
base_API iAITKIO::ImagePointer allocateImage(int const size[iAITKIO::m_DIM], double const spacing[iAITKIO::m_DIM], itk::ImageIOBase::IOComponentType type);
base_API void storeImage(iAITKIO::ImagePtr image, QString const & filename, bool useCompression);
...
base_API double itkPixel(iAITKIO::ImagePointer img, iAITKIO::ImageBaseType::IndexType idx);
base_API void setITKPixel(iAITKIO::ImagePointer img, iAITKIO::ImageBaseType::IndexType idx, double value);
//! @}

//! extract part of an image as a new file
base_API iAITKIO::ImagePointer extractImage(iAITKIO::ImagePointer inImg, size_t const indexArr[iAITKIO::m_DIM], size_t const sizeArr[iAITKIO::m_DIM]);

//! set index offset of an image to (0,0,0)
//base_API iAITKIO::ImagePointer setIndexOffsetToZero(iAITKIO::ImagePointer inImg);
template <typename T>
...
base_API void getStatistics(iAITKIO::ImagePointer img, double* min, double* max = nullptr, double* mean = nullptr, double* stddev = nullptr, double* variance = nullptr, double * sum = nullptr);
