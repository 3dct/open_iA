...
base_API vtkSmartPointer<vtkImageData> allocateImage(vtkSmartPointer<vtkImageData> img);
...
//!         with allocated but uninitialized pixel data.
base_API vtkSmartPointer<vtkImageData> allocateImage(int vtkType, int const dimensions[3], double const spacing[3]);
...
//!         with allocated but uninitialized pixel data.
base_API vtkSmartPointer<vtkImageData> allocateImage(int vtkType, int const dimensions[3], double const spacing[3], int numComponents);
...
base_API void storeImage(vtkSmartPointer<vtkImageData> img, QString const & filename, bool useCompression = true);
...
base_API vtkSmartPointer<vtkImageData> readImage(QString const & filename, bool releaseFlag);
...
base_API void writeSingleSliceImage(QString const & filename, vtkImageData* img);
...
base_API size_t mapVTKTypeToSize(int vtkType);
...
base_API bool isVtkIntegerImage(vtkImageData* img);

//! Returns a human-readable list of available data types for a single pixel/voxel.
//! @param withLongLongTypes
base_API QStringList const & readableDataTypeList(bool withLongLongTypes);
...
//!        is not on the list
base_API int mapReadableDataTypeToVTKType(QString const & dataTypeName);
...
base_API QString mapVTKTypeToReadableDataType(int vtkType);


base_API QMap<int, QString> const & RenderModeMap();

base_API int mapRenderModeToEnum(QString const &);
...
base_API void setCamPosition(vtkCamera* cam, iACameraPosition mode);
