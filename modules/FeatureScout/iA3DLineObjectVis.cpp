...
void iA3DLineObjectVis::updateValues(std::vector<std::vector<double> > const & values, int straightOrCurved)
{
	...
	{
		LOG(lvlInfo, QString("More values (current number: %1) than VTK can handle (limit: %2)")
			.arg(2 * values.size() + 1)
	...
	for (size_t f = 0; f < values.size(); ++f)
	{
		...
		else if (straightOrCurved == 2) // CurvedStepData
		{
			...
			{
				LOG(lvlInfo, QString("For fiber %1, number of points given "
		...
		{
			LOG(lvlInfo, QString("Invalid straightOrCurved value (%1) in updateValues, expected 1 or 2").arg(straightOrCurved));
		}
	}
	m_points->Modified();
	updatePolyMapper();
