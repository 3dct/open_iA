	...
	QSharedPointer<iAAttributes> attributes)
{
	...
	if (!ok)
	{
		...
		if (!ok)
		{
			LOG(lvlInfo, QString("Invalid result ID: %1").arg(tokens[0]));
			return QSharedPointer<iASingleResult>();
		}
		else
		{
			LOG(lvlInfo, "Legacy format .sps/.chr files detected, consider replacing ' ' by ',' in those files!");
		}
	}
	...
	{
		LOG(lvlInfo, QString("Invalid token count(=%1), expected %2").arg(tokens.size()).arg(attributes->size()+1));
		return QSharedPointer<iASingleResult>();
	}
	for (int i = 0; i < attributes->size(); ++i)
	{
		...
		if (!ok)
		{
			LOG(lvlInfo, QString("Could not parse attribute value # %1: '%2' (type=%3).")
...
QString iASingleResult::toString(QSharedPointer<iAAttributes> attributes, int type)
{
	...
	{
		LOG(lvlInfo, "Non-matching attribute list given (number of descriptors and number of values don't match).");
		return result;
	}
...
bool iASingleResult::loadLabelImage()
{
	...
	if (!f.exists() || f.isDir())
	{
		LOG(lvlInfo, QString("Label Image %1 does not exist, or is not a file!").arg(labelPath()));
		return false;
	}
...
void iASingleResult::setAttribute(int id, double value)
{
	...
	{
		// LOG(lvlInfo, QString("Set attribute idx (=%1) > current size (=%2)\n").arg(id).arg(m_attributeValues.size()));
