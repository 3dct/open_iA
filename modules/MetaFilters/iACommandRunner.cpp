...
void iACommandRunner::performWork()
{
	...
	else
	{
		...
		if (!success())
		{
			LOG(lvlInfo, QString("Program exited with status code %1").arg(myProcess.exitCode()));
		}
	}
...
{
	LOG(lvlInfo, QString("CommandRunner: An error has occured %1").arg
