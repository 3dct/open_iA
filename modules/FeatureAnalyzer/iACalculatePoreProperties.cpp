...
void iACalculatePoreProperties::LoadSettings()
{
	...
	csvFilename->setText( settings.value( "FeatureAnalyzer/PorePropertiesComputation/masksCSV", "" ).toString() );
}

void iACalculatePoreProperties::SaveSettings()
{
	QSettings settings( organisationName, applicationName );
	settings.setValue( "FeatureAnalyzer/PorePropertiesComputation/masksCSV", csvFilename->text() );
}
