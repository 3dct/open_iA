	...
	if (labelVoters > 0)
	{
		...
		for (int l = 0; l<labelCount; ++l)
		{
			...
			for (int m = 0; m < selection.size(); ++m)
			{
				...
				if (attributeID == -1)
				{
					LOG(lvlInfo, QString("Attribute 'Dice %1' not found, aborting!").arg(l));
	...
	int labelVoters, int weightType, int labelCount, bool undecidedPixels, double & undecided)
{
	if (selection.size() == 0)
	{
		LOG(lvlInfo, "Please select a cluster from the tree!");
...
void dlg_Consensus::LabelVoters(int)
{
	if (!m_groundTruthImage)
	{
		LOG(lvlInfo, "Please load a reference image first!");
		return;
	}
	QVector<QSharedPointer<iASingleResult> > selection;
	m_dlgGEMSe->GetSelection(selection);
...
void dlg_Consensus::StoreResult()
{
	if (!m_lastMVResult)
	{
		LOG(lvlInfo, "You need to perform Voting at least once, before last Consensus result can be stored!");
		return;
	}
	...
	if (weightType == LabelBased)
	{
		for (int l = 0; l < m_labelCount; ++l)
		{
			...
			for (int m = 0; m < selection.size(); ++m)
			{
				...
				if (attributeID == -1)
				{
					LOG(lvlInfo, QString("Attribute '%1' not found!").arg(derivedOutName));
				}
				double labelDice = selection[m]->attribute(attributeID);
...
namespace
{
	void AddParameterSets(QVector<QSet<int> > & ids, QStringList const & fullIDStrList)
	{
		...
		for (QString fullID: fullIDStrList)
		{
			...
			{
				LOG(lvlInfo, QString("Error in converting full ID '%1'!").arg(fullID));
			}
		}
	}
}
	...
	if (m_comparisonWeightType == LabelBased)
	{
		...
		for (int l = 0; l < m_labelCount; ++l)
		{
			...
			for (int m = 0; m < mvParamSetsList.size(); ++m)
			{
				...
				if (!ok)
				{
					LOG(lvlInfo, QString("Error in label weights for label %1, entry %2('%3')").arg(l).arg(m).arg(inputWeights[m]));
					return;
				}
				inputLabelWeightMap.insert(
...
void dlg_Consensus::samplerFinished()
{
	...
	if (!sender)
	{
		LOG(lvlInfo, "Invalid samplingFinished: No iAImageSampler sender!");
		return;
	}
	if (sender->isAborted())
	{
		LOG(lvlInfo, "Parameter sampling was aborted, aborting further configuration loading steps!");
		return;
	}
...
void dlg_Consensus::AddResult(vtkSmartPointer<vtkTable> table, QString const & title)
{
	...
	{
		LOG(lvlInfo, "Results vector and table are out of sync!");
		return;
	}
...
void dlg_Consensus::Sample(QVector<QSharedPointer<iASingleResult> > const & selection, int selectedClusterID, int weightType)
{
	...
	{
		LOG(lvlInfo, QString("Exception occured while sampling loaded config: %1").arg(e.what()));
	}
}
