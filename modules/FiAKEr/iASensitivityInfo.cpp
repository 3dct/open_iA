...
iASensitivityData::iASensitivityData(QSharedPointer<iAFiberResultsCollection> data,
	QStringList const& paramNames, std::vector<std::vector<double>> const& paramValues):
	m_data(data),
	...
	QVector<int> const & charSelected, QVector<int> const & charDiffMeasure, iASettings const & projectFile)
{
	...
	if (!QFile::exists(sens->dissimilarityMatrixCacheFileName()))
	{
...
void iASensitivityInfo::compute(iAProgress* progress)
{
	...
	for (size_t p = 0; p < m_data->m_paramValues[0].size(); p += m_data->m_starGroupSize)
	{
		QVector<double> parameterSet;
		for (size_t v = 0; v < m_data->m_paramValues.size(); ++v)
		{
			parameterSet.push_back(m_data->m_paramValues[v][p]);
		}
		m_data->paramSetValues.push_back(parameterSet);
		progress->emitProgress(static_cast<int>(100 * p / m_data->m_paramValues[0].size()));
	}

	progress->setStatus("Computing characteristics distribution (histogram) for all results.");
	// TODO: common storage for that in data!
	m_data->m_charHistograms.resize(static_cast<int>(m_data->m_data->result.size()));

	int numCharSelected = m_data->m_charSelected.size();
	/*
	...
	*/
	...
	for (size_t rIdx = 0; rIdx < m_data->m_data->result.size(); ++rIdx)
	{
		progress->emitProgress(static_cast<int>(100 * rIdx / m_data->m_data->result.size()));
		auto const& r = m_data->m_data->result[rIdx];
		// TODO: skip some columns? like ID...
		m_data->m_charHistograms[static_cast<qvectorsizetype>(rIdx)].reserve(numCharSelected);
		for (auto charIdx : m_data->m_charSelected)
		{
			// make sure of all histograms for the same characteristic have the same range
			double rangeMin = m_data->m_data->spmData->paramRange(charIdx)[0];
			double rangeMax = m_data->m_data->spmData->paramRange(charIdx)[1];
			...
			auto histogram = createHistogram(fiberData, m_data->m_histogramBins, rangeMin, rangeMax);
			m_data->m_charHistograms[static_cast<qvectorsizetype>(rIdx)].push_back(histogram);
		}
	}
	if (m_aborted)
	{
	...
	m_data->paramStep.fill(0.0, m_data->m_variedParams.size());
	m_data->sensitivityField.resize(numCharSelected);
	m_data->aggregatedSensitivities.resize(numCharSelected);
	for (int charIdx = 0; charIdx < numCharSelected && !m_aborted; ++charIdx)
	{
		...
		m_data->sensitivityField[charIdx].resize(m_data->m_charDiffMeasure.size());
		m_data->aggregatedSensitivities[charIdx].resize(m_data->m_charDiffMeasure.size());
		for (int diffMeasureIdx = 0; diffMeasureIdx < m_data->m_charDiffMeasure.size(); ++diffMeasureIdx)
		{
			//LOG(lvlDebug, QString("    Difference Measure %1 (%2)").arg(diffMeasure).arg(DistributionDifferenceMeasureNames()[diffMeasure]));
			auto& field = m_data->sensitivityField[charIdx][diffMeasureIdx];
			field.resize(NumOfVarianceAggregation);
			auto& agg = m_data->aggregatedSensitivities[charIdx][diffMeasureIdx];
			agg.resize(NumOfVarianceAggregation);
			for (int i = 0; i < NumOfVarianceAggregation; ++i)
			{
				field[i].resize(m_data->m_variedParams.size());
				agg[i].fill(0.0, m_data->m_variedParams.size());
			}
			for (int paramIdx = 0; paramIdx < m_data->m_variedParams.size(); ++paramIdx)
			{
				for (int i = 0; i < NumOfVarianceAggregation; ++i)
				{
					field[i][paramIdx].resize(m_data->paramSetValues.size());
				}
				...
				int origParamColIdx = m_data->m_variedParams[paramIdx];
				// aggregation types:
				...
				int numAllLeft = 0,
					...
					numAllLeftRight = 0,
					numAllTotal = 0;
				for (int paramSetIdx = 0; paramSetIdx < m_data->paramSetValues.size(); ++paramSetIdx)
				{
					int resultIdxGroupStart = m_data->m_starGroupSize * paramSetIdx;
					int resultIdxParamStart = resultIdxGroupStart + 1 + paramIdx * m_data->m_numOfSTARSteps;

					// first - then + steps (both skipped if value +/- step exceeds bounds
					double groupStartParamVal = m_data->m_paramValues[origParamColIdx][resultIdxGroupStart];
					double paramStartParamVal = m_data->m_paramValues[origParamColIdx][resultIdxParamStart];
					...
					if (m_data->paramStep[paramIdx] == 0)
					{
						m_data->paramStep[paramIdx] = std::abs(paramDiff);
						LOG(lvlInfo, QString("Param %1 step width: %2").arg(paramIdx).arg(m_data->paramStep[paramIdx]));
					}
					...
					if (paramDiff > 0)
					{
						leftVar = distributionDifference(
							m_data->m_charHistograms[resultIdxGroupStart][charIdx],
							m_data->m_charHistograms[resultIdxParamStart][charIdx],
							m_data->m_charDiffMeasure[diffMeasureIdx]);
					...
					int k = 1;
					while (paramDiff > 0 && k < m_data->m_numOfSTARSteps)
					{
						double paramVal = m_data->m_paramValues[origParamColIdx][resultIdxParamStart + k];
					...
					if (paramDiff < 0) // additional check required??
					{
						...
						rightVar = distributionDifference(
							m_data->m_charHistograms[resultIdxGroupStart][charIdx],
							m_data->m_charHistograms[firstPosStepIdx][charIdx],
							m_data->m_charDiffMeasure[diffMeasureIdx]);
					...
					for (int i = 0; i < m_data->m_numOfSTARSteps; ++i)
					{
						int compareIdx = (i==0)? resultIdxGroupStart : (resultIdxParamStart + i - 1);
						double paramVal = m_data->m_paramValues[origParamColIdx][resultIdxParamStart + i];
						...
						double difference = distributionDifference(
							m_data->m_charHistograms[compareIdx][charIdx],
							m_data->m_charHistograms[resultIdxParamStart + i][charIdx],
							m_data->m_charDiffMeasure[diffMeasureIdx]);
						sumTotal += difference;
					}
					numAllLeftRight += numLeftRight;
					numAllTotal += m_data->m_numOfSTARSteps;
					double meanLeftRightVar = (leftVar + rightVar) / numLeftRight;
					double meanTotal = sumTotal / m_data->m_numOfSTARSteps;
	...
	for (size_t resultIdx = 0; resultIdx < m_data->m_data->result.size(); ++resultIdx)
	{
		fiberCounts.push_back(m_data->m_data->result[resultIdx].fiberCount);
	}
	m_data->m_fiberCountRange[0] = m_data->m_fiberCountRange[1] = std::numeric_limits<double>::infinity();
	m_data->fiberCountHistogram = createHistogram(fiberCounts, m_data->m_histogramBins,
		m_data->m_fiberCountRange[0], m_data->m_fiberCountRange[1], true);
	if (m_aborted)
	{
		return;
	}
	...
	// TODO: unify with other loops over STARs
	m_data->sensitivityFiberCount.resize(NumOfVarianceAggregation);
	m_data->aggregatedSensitivitiesFiberCount.resize(NumOfVarianceAggregation);
	for (int i = 0; i < NumOfVarianceAggregation; ++i)
	{
		m_data->sensitivityFiberCount[i].resize(m_data->m_variedParams.size());
		m_data->aggregatedSensitivitiesFiberCount[i].fill(0.0, m_data->m_variedParams.size());
	}
	for (int paramIdx = 0; paramIdx < m_data->m_variedParams.size() && !m_aborted; ++paramIdx)
	{
		progress->emitProgress(100 * paramIdx / m_data->m_variedParams.size());
		int origParamColIdx = m_data->m_variedParams[paramIdx];
		for (int i = 0; i < NumOfVarianceAggregation; ++i)
		{
			m_data->sensitivityFiberCount[i][paramIdx].resize(m_data->paramSetValues.size());
		}
		int numAllLeft = 0,
			numAllRight = 0,
			numAllLeftRight = 0,
		...
		for (int paramSetIdx = 0; paramSetIdx < m_data->paramSetValues.size(); ++paramSetIdx)
		{
			...
			if (paramDiff > 0)
			{
				leftVar = std::abs(static_cast<double>(m_data->m_data->result[resultIdxGroupStart].fiberCount) -
					m_data->m_data->result[resultIdxParamStart].fiberCount);
				//LOG(lvlDebug, QString("        Left var available: %1").arg(leftVar));
				++numLeftRight;
				++numAllLeft;
			}
			...
			{
				int firstPosStepIdx = resultIdxParamStart + (k - 1);
				rightVar = std::abs(static_cast<double>(m_data->m_data->result[resultIdxGroupStart].fiberCount) -
					m_data->m_data->result[firstPosStepIdx].fiberCount);
				//LOG(lvlDebug, QString("        Right var available: %1").arg(rightVar));
				++numLeftRight;
				++numAllRight;
			}
			...
			for (int i = 0; i < m_data->m_numOfSTARSteps; ++i)
			{
				...
				double difference = std::abs(static_cast<double>(m_data->m_data->result[compareIdx].fiberCount) -
					m_data->m_data->result[resultIdxParamStart + i].fiberCount);
				sumTotal += difference;
			}
			numAllLeftRight += numLeftRight;
			numAllTotal += m_data->m_numOfSTARSteps;
			...
			//LOG(lvlDebug, QString("        (sum total var = %1) / (m_numOfSTARSteps = %2)  = %3")
			//	.arg(sumTotal).arg(m_numOfSTARSteps).arg(meanTotal));
			m_data->sensitivityFiberCount[0][paramIdx][paramSetIdx] = meanLeftRightVar;
			m_data->sensitivityFiberCount[1][paramIdx][paramSetIdx] = leftVar;
			m_data->sensitivityFiberCount[2][paramIdx][paramSetIdx] = rightVar;
			m_data->sensitivityFiberCount[3][paramIdx][paramSetIdx] = meanTotal;

			m_data->aggregatedSensitivitiesFiberCount[0][paramIdx] += meanLeftRightVar;
			m_data->aggregatedSensitivitiesFiberCount[1][paramIdx] += leftVar;
			m_data->aggregatedSensitivitiesFiberCount[2][paramIdx] += rightVar;
			m_data->aggregatedSensitivitiesFiberCount[3][paramIdx] += meanTotal;
		}
	}
	if (m_aborted)
	{
	...
	m_data->charHistVar.resize(numCharSelected);
	m_data->charHistVarAgg.resize(numCharSelected);
	for (int charIdx = 0; charIdx < numCharSelected && !m_aborted; ++charIdx)
	{
		progress->emitProgress(100 * charIdx / numCharSelected);
		//charHistHist[charIdx].resize(NumOfVarianceAggregation);
		m_data->charHistVar[charIdx].resize(NumOfVarianceAggregation);
		m_data->charHistVarAgg[charIdx].resize(NumOfVarianceAggregation);
		for (int aggIdx = 0; aggIdx < NumOfVarianceAggregation && !m_aborted; ++aggIdx)
		{
			//charHistHist[charIdx][aggIdx].resize(m_variedParams.size());
			m_data->charHistVar[charIdx][aggIdx].resize(m_data->m_variedParams.size());
			m_data->charHistVarAgg[charIdx][aggIdx].resize(m_data->m_variedParams.size());
		}
		for (int paramIdx = 0; paramIdx < m_data->m_variedParams.size() && !m_aborted; ++paramIdx)
		{
			for (int aggIdx = 0; aggIdx < NumOfVarianceAggregation && !m_aborted; ++aggIdx)
			{
				...
				m_data->charHistVar[charIdx][aggIdx][paramIdx].resize(m_data->m_histogramBins);
				m_data->charHistVarAgg[charIdx][aggIdx][paramIdx].fill(0.0, m_data->m_histogramBins);
			}
			// TODO: unify with other loops over STARs?
			int origParamColIdx = m_data->m_variedParams[paramIdx];

			for (int bin = 0; bin < m_data->m_histogramBins; ++bin)
			{
				int numAllLeft = 0,
					numAllRight = 0,
					numAllTotal = 0;
				for (int aggIdx = 0; aggIdx < NumOfVarianceAggregation && !m_aborted; ++aggIdx)
				{
					...
					m_data->charHistVar[charIdx][aggIdx][paramIdx][bin].resize(m_data->paramSetValues.size());
				}
				for (int paramSetIdx = 0; paramSetIdx < m_data->paramSetValues.size(); ++paramSetIdx)
				{
					int resultIdxGroupStart = m_data->m_starGroupSize * paramSetIdx;
					...
					*/
					m_data->charHistVar[charIdx][0][paramIdx][bin][paramSetIdx] = 0;
					if (paramDiff > 0)
					{
						// left-only:
						...
						m_data->charHistVar[charIdx][0][paramIdx][bin][paramSetIdx] =
							std::abs(m_data->m_charHistograms[resultIdxGroupStart][charIdx][bin] - m_data->m_charHistograms[resultIdxParamStart][charIdx][bin]);
						m_data->charHistVarAgg[charIdx][0][paramIdx][bin] += m_data->charHistVar[charIdx][0][paramIdx][bin][paramSetIdx];
						m_data->charHistVarAgg[charIdx][2][paramIdx][bin] += m_data->charHistVar[charIdx][0][paramIdx][bin][paramSetIdx];
						//LOG(lvlDebug, QString("        Left var available: %1").arg(leftVar));
						++numLeftRight;
						++numAllLeft;
					}
					...
					m_data->charHistVar[charIdx][1][paramIdx][bin][paramSetIdx] = 0;
					if (paramDiff < 0) // additional check required??
					{
						int firstPosStepIdx = resultIdxParamStart + (k - 1);
						// left-only:
						...
						m_data->charHistVar[charIdx][1][paramIdx][bin][paramSetIdx] =
							std::abs(m_data->m_charHistograms[resultIdxGroupStart][charIdx][bin] - m_data->m_charHistograms[firstPosStepIdx][charIdx][bin]);
						m_data->charHistVarAgg[charIdx][1][paramIdx][bin] += m_data->charHistVar[charIdx][1][paramIdx][bin][paramSetIdx];
						m_data->charHistVarAgg[charIdx][2][paramIdx][bin] += m_data->charHistVar[charIdx][1][paramIdx][bin][paramSetIdx];
						//LOG(lvlDebug, QString("        Right var available: %1").arg(rightVar));
						++numLeftRight;
						++numAllRight;
					}
					m_data->charHistVar[charIdx][2][paramIdx][bin][paramSetIdx] /= numLeftRight;
					bool wasSmaller = true;
					m_data->charHistVar[charIdx][3][paramIdx][bin][paramSetIdx] = 0;
					numAllTotal += m_data->m_numOfSTARSteps;
					for (int i = 0; i < m_data->m_numOfSTARSteps; ++i)
					{
						...
						m_data->charHistVar[charIdx][3][paramIdx][bin][paramSetIdx] +=
							std::abs(m_data->m_charHistograms[compareIdx][charIdx][bin] -
								m_data->m_charHistograms[resultIdxParamStart + i][charIdx][bin]);
					}
					m_data->charHistVar[charIdx][3][paramIdx][bin][paramSetIdx] /= m_data->m_numOfSTARSteps;//charHistHist[charIdx][3][paramIdx][bin][paramSetIdx].size();
					m_data->charHistVarAgg[charIdx][3][paramIdx][bin] += m_data->charHistVar[charIdx][3][paramIdx][bin][paramSetIdx];
				}
				assert(numAllTotal == m_data->paramSetValues.size() * m_data->m_numOfSTARSteps);
				m_data->charHistVarAgg[charIdx][0][paramIdx][bin] /= numAllLeft;
				m_data->charHistVarAgg[charIdx][1][paramIdx][bin] /= numAllRight;
				m_data->charHistVarAgg[charIdx][2][paramIdx][bin] /= (numAllLeft + numAllRight);
				m_data->charHistVarAgg[charIdx][3][paramIdx][bin] /= numAllTotal;
			}
		}
	}
	if (m_aborted)
	...
	m_data->charHistAvg.resize(numCharSelected);
	for (int charIdx = 0; charIdx < numCharSelected && !m_aborted; ++charIdx)
	{
		m_data->charHistAvg[charIdx].fill(0, m_data->m_histogramBins);
		for (int resultIdx = 0; resultIdx < m_data->m_charHistograms.size(); ++resultIdx)
		{
			for (int bin = 0; bin < m_data->m_histogramBins; ++bin)
			{
				m_data->charHistAvg[charIdx][bin] += m_data->m_charHistograms[resultIdx][charIdx][bin];
			}
		}
		for (int bin = 0; bin < m_data->m_histogramBins; ++bin)
		{
			m_data->charHistAvg[charIdx][bin] /= m_data->m_charHistograms.size();
		}
	}
	...
	{
		m_data->m_resultDissimMeasures.clear();
		for (auto m : measures)
		{
			m_data->m_resultDissimMeasures.push_back(std::make_pair(m, true));
		}
	}
	else
	{
		progress->setStatus("Computing dissimilarity between all result pairs.");
		int measureCount = static_cast<int>(m_data->m_resultDissimMeasures.size());
		int resultCount = static_cast<int>(m_data->m_data->result.size());
		m_data->m_resultDissimMatrix = iADissimilarityMatrixType(
			resultCount,
			QVector<iAResultPairInfo>(resultCount, iAResultPairInfo(measureCount)));

		for (size_t m = 0; m < m_data->m_resultDissimMeasures.size(); ++m)
		{
			measures.push_back(m_data->m_resultDissimMeasures[m].first);
		}
		...
		{
			auto& res1 = m_data->m_data->result[r1];
			auto const& mapping = *res1.mapping.data();
			// TODO: only center -> should use bounding box instead!
			double const *cxr = m_data->m_data->spmData->paramRange(mapping[iACsvConfig::CenterX]),
				* cyr = m_data->m_data->spmData->paramRange(mapping[iACsvConfig::CenterY]),
				* czr = m_data->m_data->spmData->paramRange(mapping[iACsvConfig::CenterZ]);
			double a = cxr[1] - cxr[0], b = cyr[1] - cyr[0], c = czr[1] - czr[0];
			double diagonalLength = std::sqrt(std::pow(a, 2) + std::pow(b, 2) + std::pow(c, 2));
			double const* lengthRange = m_data->m_data->spmData->paramRange(mapping[iACsvConfig::Length]);
			double maxLength = lengthRange[1] - lengthRange[0];
			for (int r2 = 0; r2 < resultCount && !m_aborted; ++r2)
			{
				auto& mat = m_data->m_resultDissimMatrix[r1][r2];
				...
				int r1FibCount = static_cast<int>(m_data->m_data->result[r1].fiberCount);
...
#pragma omp parallel for reduction(+ : noCanDo, candSum)
				...
				{
					auto candidates = intersectingBoundingBox(m_data->m_data->result[r1].fiberBB[fiberID], m_data->m_data->result[r2].fiberBB);
					...
					getBestMatches2(m_data->m_data->result[r1].fiberData[fiberID], m_data->m_data->result[r2].fiberData,
						dissimilarities[fiberID], candidates, diagonalLength, maxLength,
						m_data->m_resultDissimMeasures);
					for (int m = 0; m < measureCount; ++m)
					{
	...
	if (m_data->m_resultDissimMatrix.size() == 0)
	{
	...
	m_data->m_resultDissimRanges.resize(static_cast<qvectorsizetype>(m_data->m_resultDissimMeasures.size()));
	for (int m = 0; m < m_data->m_resultDissimRanges.size(); ++m)
	{
		m_data->m_resultDissimRanges[m].first = std::numeric_limits<double>::max();
		m_data->m_resultDissimRanges[m].second = std::numeric_limits<double>::lowest();
	}
	for (qvectorsizetype r1 = 1; r1 < static_cast<qvectorsizetype>(m_data->m_data->result.size()) && !m_aborted; ++r1)
	{
		for (qvectorsizetype r2 = 0; r2 < static_cast<qvectorsizetype>(m_data->m_data->result.size()) && !m_aborted; ++r2)
		{
			if (r1 == r2)
			{
				continue;
			...
			for (int m = 0; m < m_data->m_resultDissimRanges.size(); ++m)
			{
				double dissim = m_data->m_resultDissimMatrix[r1][r2].avgDissim[m];
				if (dissim < m_data->m_resultDissimRanges[m].first)
				{
					m_data->m_resultDissimRanges[m].first = dissim;
				}
				if (dissim > m_data->m_resultDissimRanges[m].second)
				{
					m_data->m_resultDissimRanges[m].second = dissim;
				}
			}
		}
	}
	for (int m = 0; m < m_data->m_resultDissimRanges.size(); ++m)
	{
		LOG(lvlDebug,
			QString("m: %1; range: %2..%3")
				.arg(m)
				.arg(m_data->m_resultDissimRanges[m].first)
				.arg(m_data->m_resultDissimRanges[m].second));
	}
	if (m_aborted)
	{
		return;
	...
	int measureCount = static_cast<int>(m_data->m_resultDissimMeasures.size());
		// TODO: unify with other loops over STARs
	m_data->sensDissimField.resize(measureCount);
	m_data->aggregatedSensDissim.resize(measureCount);

	for (int m = 0; m < measureCount && !m_aborted; ++m)
	{
		QVector<double> dissimValuesUsed;
		m_data->sensDissimField[m].resize(NumOfVarianceAggregation);
		m_data->aggregatedSensDissim[m].resize(NumOfVarianceAggregation);
		for (int a = 0; a < NumOfVarianceAggregation; ++a)
		{
			m_data->sensDissimField[m][a].resize(m_data->m_variedParams.size());
			m_data->aggregatedSensDissim[m][a].fill(0.0, m_data->m_variedParams.size());
		}
		for (int paramIdx = 0; paramIdx < m_data->m_variedParams.size() && !m_aborted; ++paramIdx)
		{
			progress->emitProgress(100 * paramIdx / m_data->m_variedParams.size());
			...
			for (int a = 0; a < NumOfVarianceAggregation; ++a)
			{
				m_data->sensDissimField[m][a][paramIdx].resize(m_data->paramSetValues.size());
			}
			...
			for (int paramSetIdx = 0; paramSetIdx < m_data->paramSetValues.size(); ++paramSetIdx)
			{
				...
				if (paramDiff > 0)
				{
					leftVar = m_data->m_resultDissimMatrix[resultIdxGroupStart][resultIdxParamStart].avgDissim[m];
				...
				{
					int firstPosStepIdx = resultIdxParamStart + (k - 1);
					rightVar = m_data->m_resultDissimMatrix[resultIdxGroupStart][firstPosStepIdx].avgDissim[m];
				...
				for (int i = 0; i < m_data->m_numOfSTARSteps; ++i)
				{
					...
					double difference = m_data->m_resultDissimMatrix[compareIdx][resultIdxParamStart + i].avgDissim[m];
				...
				//LOG(lvlDebug, QString("        (sum total var = %1) / (m_numOfSTARSteps = %2)  = %3")
				//	.arg(sumTotal).arg(m_numOfSTARSteps).arg(meanTotal));
				m_data->sensDissimField[m][0][paramIdx][paramSetIdx] = meanLeftRightVar;
				m_data->sensDissimField[m][1][paramIdx][paramSetIdx] = leftVar;
				m_data->sensDissimField[m][2][paramIdx][paramSetIdx] = rightVar;
				m_data->sensDissimField[m][3][paramIdx][paramSetIdx] = meanTotal;

				m_data->aggregatedSensDissim[m][0][paramIdx] += meanLeftRightVar;
				m_data->aggregatedSensDissim[m][1][paramIdx] += leftVar;
				m_data->aggregatedSensDissim[m][2][paramIdx] += rightVar;
				m_data->aggregatedSensDissim[m][3][paramIdx] += meanTotal;
			}
		}
		QPair<double,double> dissimRange;
		dissimRange.first = dissimRange.second = std::numeric_limits<double>::infinity();
		auto dissimHistogram = createHistogram(dissimValuesUsed, m_data->m_histogramBins, dissimRange.first, dissimRange.second, false);
		m_data->m_dissimRanges.push_back(dissimRange);
		m_data->m_dissimHistograms.push_back(dissimHistogram);
	}
}

QString iASensitivityInfo::cacheFileName(QString fileName) const
{
	return QString("%1/cache/%2").arg(m_data->m_data->folder).arg(fileName);
}
...
bool iASensitivityInfo::readDissimilarityMatrixCache(QVector<int>& measures)
{
	...
	in >> measures;
	in >> m_data->m_resultDissimMatrix;
	cacheFile.close();
	return true;
}
...
void iASensitivityInfo::writeDissimilarityMatrixCache(QVector<int> const& measures) const
{
	...
	out << measures;
	out << m_data->m_resultDissimMatrix;
}
...
*/
	...
	size_t findMatch(iADissimilarityMatrixType const& dissimMatrix, iASensitivityData::UniqueFibersT const& uniqueFibers,
...
void iASensitivityInfo::computeSpatialOverview(iAProgress * progress)
{
	...
	for (size_t r = 0; r < resultCount; ++r)
	{
		resultBBs.push_back(m_data->m_data->result[r].bbox);
	}
	iAAABB overallBB;
	...
	for (size_t r = 0; r < resultCount && !m_aborted; ++r)
	{
		...
		auto const& d = m_data->m_data->result[r];
	...
	for (size_t r1 = 0; r1 < resultCount && !m_aborted; ++r1)
	{
		...
		for (size_t f1 = 0; f1 < d.fiberCount; ++f1)
		{
			...
			auto idx = findMatch(m_data->m_resultDissimMatrix, m_data->m_uniqueFibers, m_data->m_mapFiberToUnique, curFiber, MeasureIdx);
			if (idx != NoMatch)
			{
				m_data->m_uniqueFibers[idx].push_back(curFiber);
				m_data->m_mapFiberToUnique.insert(curFiber, idx);
			}
			...
			else
			{
				...
				m_data->m_uniqueFibers.push_back(vec);
				m_data->m_mapFiberToUnique.insert(curFiber, m_data->m_uniqueFibers.size() - 1);
			}
		}
	...
	LOG(lvlDebug, QString("Found %1 unique fibers across results!").arg(m_data->m_uniqueFibers.size()));
	...
	for (size_t uIdx = 0; uIdx < m_data->m_uniqueFibers.size() && !m_aborted; ++uIdx)
	{
		...
		progress->emitProgress(100 * (uIdx + 1) / m_data->m_uniqueFibers.size());
	}
	multiplyImage(m_spatialOverview, m_data->m_uniqueFibers.size());
	h.stop();
