...
void iAFiAKErController::loadProject(QSettings const& projectFile, QString const& fileName)
{
	...
	{  // load full format
		...
		{
			LOG(lvlInfo, "Could not load CSV format specification from project file!");
			return;
		}
	}
...
bool readParameterCSV(QString const& fileName, QString const & encoding, QString const & columnSeparator, iACsvTableCreator& tblCreator, size_t resultCount)
{
	...
	{
		LOG(lvlInfo, QString("Unable to open file '%1': %2").arg(fileName).arg(file.errorString()));
		return false;
	}
	QTextStream in(&file);
...
public:
		...
		iADissimilarityMatrixType const& dissimMatrix)
	{
		...
		paramListLayout->setSpacing(ControlSpacing);
		paramListLayout->setContentsMargins(ResultListMargin, ResultListMargin, ResultListMargin, ResultListMargin);
...
QString iAFiAKErController::dissimilarityMatrixCacheFileName()
{
...
bool iAFiAKErController::readDissimilarityMatrixCache(QVector<int>& measures)
{
	...
	{
		LOG(lvlInfo, QString("Couldn't open file %1 for reading!").arg(cacheFile.fileName()));
		return false;
	}
	...
	{
		LOG(lvlInfo, QString("FIAKER cache file '%1': Unknown cache file format - found identifier %2 does not match expected identifier %3.")
	...
	{
		LOG(lvlInfo, QString("FIAKER cache file '%1': Invalid or too high version number (%2), expected %3 or less.")
			.arg(cacheFile.fileName())
			.arg(version).arg(DissimilarityMatrixCacheFileVersion));
		return false;
	}
	in >> measures;
	in >> m_dissimilarityMatrix;
	return true;
}

void iAFiAKErController::writeDissimilarityMatrixCache(QVector<int> const& measures)
{
	QFile cacheFile(dissimilarityMatrixCacheFileName());
	if (!cacheFile.open(QFile::WriteOnly))
	{
		LOG(lvlInfo, QString("Couldn't open file %1 for writing!").arg(cacheFile.fileName()));
		return;
	}
...
void iAFiAKErController::sensitivitySlot()
{
	...
	if (!readDissimilarityMatrixCache(measures))
	{
		...
		auto measuresComputed = selectMeasure.measures();
		auto optimizationMeasureIdx = selectMeasure.optimizeMeasureIdx();

		m_dissimilarityMatrix = iADissimilarityMatrixType(m_data->result.size(),
			QVector<iAResultPairInfo>(m_data->result.size(),
				iAResultPairInfo(measuresComputed.size())));
		for (size_t m = 0; m < measuresComputed.size(); ++m)
		{
			measures.push_back(measuresComputed[m].first);
		}
...
#pragma omp parallel for
				for (qint64 fiberID = 0; fiberID < fiberCount; ++fiberID)
				{
					...
					getBestMatches(fiber, mapping, res1.table, dissimilarities[fiberID], res1.curveInfo,
						diagonalLength, maxLength, measuresComputed, optimizationMeasureIdx);
					for (size_t m = 0; m < measuresComputed.size(); ++m)
					{
...
void iAFiAKErController::dissimMatrixMeasureChanged(int idx)
{
	...
	m_parameterListView->dissimMatrixMeasureChanged(idx);
}
...
void iAFiAKErController::changeDistributionSource(int index)
{
	...
	{
		LOG(lvlInfo, QString("You need to set a reference first!"));
		return;
	}
	double range[2];
...
void iAFiAKErController::exportDissimilarities()
{
	...
	{
		LOG(lvlInfo, "No reference set, therefore there are no dissimilarities to export!");
		return;
	}
	...
	{
		LOG(lvlInfo, QString("Cannot open file %1 for writing!").arg(fileName));
		return;
	}
...
void iAFiAKErController::toggleOptimStepChart(size_t chartID, bool visible)
{
	if (!visible)
	{
		if (!m_optimStepChart[chartID])
		{
			LOG(lvlInfo, QString("Step chart %1 toggled invisible, but not created yet.").arg(chartID));
			return;
		}
		...
		for (size_t resultID=0; resultID<m_data->result.size(); ++resultID)
		{
			...
			for (size_t fiberID = 0; fiberID < d.fiberCount; ++fiberID)
			{
				...
				if (chartID < m_chartCount - 1)
				{
					...
					{
						LOG(lvlInfo, "Differences for this measure not computed (yet).");
						return;
					}
				}
				else
	...
	for (size_t resultID=0; resultID<m_data->result.size(); ++resultID)
	{
		...
		for (size_t p = 0; p < m_data->result[resultID].fiberCount; ++p)
		{
			...
			{
				LOG(lvlInfo, "Tried to show/hide unavailable plot.");
				return;
			}
		}
	}
				...
				for (size_t c = 0; c < m_chartCount; ++c)
				{
					if (m_optimStepChart[c] && m_optimStepChart[c]->isVisible())
					{
						for (size_t p = 0; p < d.fiberCount; ++p)
						{
							...
							{
								LOG(lvlInfo, QString("Invalid chart access: access to plot %1, but only has %2")
...
void iAFiAKErController::getResultFiberIDFromSpmID(size_t spmID, size_t & resultID, size_t & fiberID)
{
	...
	{
		LOG(lvlInfo, QString("Invalid index in SPM: %1").arg(spmID));
		return;
	}
...
void iAFiAKErController::updateBoundingBox()
{
	...
	{                                             // todo: error checking
		...
		if (!ok)
		{
			LOG(lvlInfo, QString("Invalid bounding box value: %1").arg(m_teBoundingBox[i]->text()))
		}
...
void iAFiAKErController::referenceToggled()
{
	if (m_refDistCompute)
	{
		LOG(lvlInfo, "Another reference computation is currently running, please let that finish first!");
		return;
	}
...
bool iAFiAKErController::loadReferenceInternal(iASettings settings)
{
	...
	if (referenceID == NoResult)
	{
		...
		{
			LOG(lvlInfo, QString("Invalid reference specification '%1' in project file! "
...
namespace
{
	...
	void setCameraParameter(iASettings const & settings, QString const & key, vtkCamera* cam, SetMethod method)
	{
		...
		{
			LOG(lvlInfo, QString("Invalid value %1 for key=%2 couldn't be parsed as double array of size 3!")
