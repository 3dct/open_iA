...
namespace
{
	std::shared_ptr<iAThemeChangeNotifier> createThemeChangeNotifier()
	{
		auto result = std::make_shared<iAThemeChangeNotifier>();
#ifdef _MSC_VER
		auto future = QtConcurrent::run(
			[result]
			{
				...
				while (running)
				{
					...
					LOG(lvlInfo, QString("Waiting for a change in the specified key..."));
					HANDLE handles[2] = {hEvent, result->m_stopEvent };
					auto waitResult = WaitForMultipleObjects(2, handles, false, INFINITE);
					if (waitResult == WAIT_OBJECT_0)
					{
						LOG(lvlInfo, "Change indicated...");
						bool newBright = iAThemeChangeNotifier::isBrightTheme();
						if (result->m_isBright != newBright)
						{
							result->m_isBright = newBright;
							LOG(lvlInfo, "    Value actually changed!");
							result->emitThemeChanged(newBright);
						}
					}
...
iAThemeChangeNotifier::iAThemeChangeNotifier()
{
	...
	m_isBright = iAThemeChangeNotifier::isBrightTheme();
}

iAThemeChangeNotifier::~iAThemeChangeNotifier()
{}

bool iAThemeChangeNotifier::isBrightTheme()
{
	/*
	...
	*/
	...
	LOG(lvlInfo, QString("isBrightTheme: %1").arg(bright));
	return bright;
}

void iAThemeChangeNotifier::stop()
{
...
iAThemeChangeNotifier* iAThemeChangeNotifier::get()
{
	static auto tcn = createThemeChangeNotifier();
	return tcn.get();
}

void iAThemeChangeNotifier::emitThemeChanged(bool brightTheme)
{
	emit themeChanged(brightTheme);
}
