...
namespace iAITKIO
{
	...
	iAio_API ImagePointer readFile(QString const& fileName, PixelType& pixelType, ScalarType& scalarType, bool releaseFlag, iAProgress const * progress = nullptr);
	...
	//! @param[in] progress optional progress reporting capabilities via iAProgress
	iAio_API void writeFile(QString const& fileName, ImagePtr image, ScalarType scalarType, bool useCompression = false, iAProgress const * progress = nullptr);
} // namespace iAITKIO

// TODO: merge with iAITKIO::writeFile
iAio_API void storeImage(iAITKIO::ImagePtr image, QString const & filename, bool useCompression, iAProgress const* p = nullptr);
